...
import PublicBlueprintCard from "@/app/components/dashboard/user/PublicBlueprintCard";
...
export default function BlueprintsPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-3 sm:px-6 py-6 sm:py-12">
      <h1 className="text-xl sm:text-3xl font-bold mb-4 sm:mb-6">Blueprints</h1>

      <BlueprintFilters value={filters} onChange={handleFilterChange} />

      {isError && <p className="text-red-500 text-sm sm:text-base">Failed to load blueprints.</p>}
      ...
      {isLoading ? (
        ...
        <p className="text-gray-500 text-sm sm:text-base">No blueprints found. Try adjusting your filters.</p>
      ) : (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
            {data?.data.map((bp) => (
