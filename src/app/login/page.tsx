...
import { FiMail, FiLock, FiEye, FiEyeOff, FiLoader, FiUser, FiShield } from "react-icons/fi";

import { loginSchema, type LoginInput } from "@/app/lib/validations/auth.schema";
...
export default function LoginPage() {
  ...
  const [demoLoadingRole, setDemoLoadingRole] = useState<"user" | "admin" | null>(null);
  ...
  async function handleDemoLogin(role: "user" | "admin") {
    const credentials = role === "user" ? DEMO_USER : DEMO_ADMIN;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Log in to continue building your learning blueprints
        </p>

        {/* Demo login — user & admin, side by side */}
        <div className="mt-6 grid grid-cols-2 gap-3">
          <button
            type="button"
            onClick={() => handleDemoLogin("user")}
            disabled={isSubmitting}
            className="flex items-center justify-center gap-1.5 rounded-lg border py-2.5 text-sm font-medium transition-colors disabled:opacity-60"
            style={{
